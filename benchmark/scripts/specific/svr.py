...
parser = argparse.ArgumentParser(description='LinearSVR Regression')
...
linear_svr = LinearSVR(random_state=seed)
pipe = Pipeline(steps=[
    ('nystroem', nystroem),
...
if args.cv:
    ...
    param_dist = {
        ...
        "nystroem__n_components": [10, 50, 100],
        # "linear_svr__C": loguniform(1e-1, 1e3),  # [1e-1, 1, 10, 100, 1e3],
        "linear_svr__C": [1e3, 1e4],  # loguniform(1e-1, 1e3),
        # "linear_svr__epsilon": loguniform(1e-4, 10),  # [1e-4, 1e-3, 1e-2, 1e-1, 1, 10],
    ...
    pipe_best = Pipeline(steps=[
        ...
        ('linear_svr', linear_svr_b)])
start1 = time()
pipe_best.fit(train_df_features.values, train_df_labels.values)
end1 = time()
train_pred = pipe_best.predict(train_df_features.values).reshape(-1)
end2 = time()
test_pred1 = pipe_best.predict(test_df1_features.values).reshape(-1)
end3 = time()
test_pred2 = pipe_best.predict(test_df2_features.values).reshape(-1)
end4 = time()
