...
if args.cv:
    ...
    rf_best = RandomForestRegressor(n_estimators=100,
                                    ...
                                    random_state=seed)
start1 = time()
rf_best.fit(train_df_features.values, train_df_labels.values)
end1 = time()
train_pred = rf_best.predict(train_df_features.values).reshape(-1)
end2 = time()
test_pred1 = rf_best.predict(test_df1_features.values).reshape(-1)
end3 = time()
test_pred2 = rf_best.predict(test_df2_features.values).reshape(-1)
end4 = time()
